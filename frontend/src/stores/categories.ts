...
export const useCategoriesStore = defineStore('categories', () =>
{
  ...
  {
    return categories.value.find(cat => cat.name_de === name);
  }
  ...
  // Generate SVG icon for marker with category color
  function getCategoryIcon(categoryNames: string[]): string
  {
  ...
  // Toggle category in a list (helper for forms)
  ...
  async function fetchCategories(): Promise<void>
  {
    ...
    if (hasFetched.value && categories.value.length > 0)
    {
      return;
    }
